...
Public Sub aheadstatusEND(a$, pos As Long, Lang As Long, second1$, len1 As Long, second2$, len2 As Long)
Dim pos2 As Long, Level&, what$, w$, lenA As Long, level2 As Integer
...
i = 1
aheadstatusThen a$, i, 1, w$
'w$ = aheadstatus(a$, , i)
Debug.Print Mid$(a$, i), w$
End Sub
...
there:
    ...
    res = -1
    For w3 = p To pppp.count - 1
    If pppp.IsStringItem(w3) Then If pppp.item(w3) = st(0) Then res = w3: Exit For
    Next w3
    Do While FastSymbol(a$, ",")
        ...
        If res > -1 Then
            ...
            If w3 < pppp.count Then
                If pppp.IsStringItem(w3) Then
                    If pppp.item(w3) <> st(cur) Then res = -1
                Else
                    res = -1
                End If
            Else
